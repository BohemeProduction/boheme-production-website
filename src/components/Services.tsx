  ...
  return (
    <section id="prestations" className="py-20 lg:py-28 bg-gradient-to-b from-gray-50 to-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Services Grid */}
        <div className="grid grid-cols-2 gap-6 sm:gap-8 mb-10 sm:mb-16 fade-in-section">
          {services.map((service, index) => {
            const Icon = service.icon;
            return (
              <div
                key={index}
                className={`group bg-white rounded-3xl p-8 shadow-xl hover:shadow-2xl transition-all duration-500 border border-gray-100 hover:border-[#e8dcc7]/30 flex flex-col h-full hover-lift animate-fade-in-up stagger-${(index % 3) + 1}`}
              >
                <div className="text-center mb-6 flex-grow">
                  <div className={`inline-flex items-center justify-center w-16 h-16 rounded-full bg-gradient-to-br ${service.bgColor} mb-6 group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 animate-pulse-slow`}>
                    <Icon className={`h-8 w-8 ${service.color}`} />
                  </div>
                  <h3 className="text-2xl font-bold text-[#13182c] mb-4">{service.title}</h3>
                  <p className="text-gray-600 leading-relaxed font-medium mb-6">{service.description}</p>
                  
                  <div className="space-y-3 text-center">
                    {service.features.map((feature, idx) => (
                      <div key={idx} className="flex items-center justify-center space-x-3 group">
                        <Eye className="h-4 w-4 mt-1 flex-shrink-0 text-[#13182c] group-hover:scale-110 transition-all duration-300" />
                        <span className="text-gray-600 font-medium text-sm leading-relaxed group-hover:text-gray-800 transition-colors duration-300">{feature}</span>
                      </div>
                    ))}
                  </div>
                </div>
